...
The C library can be built and installed with `cargo xtask install` (see `--help` for options). Optionally the build step can also be separated with `cargo xtask build`.
...
int main() {
  ...
  sbr_subtitles *subs = sbr_load_text(sbr, content, strlen(content),
                                      SBR_SUBTITLE_FORMAT_UNKNOWN, "en-US");
  if (!subs)
    exit(1);
