...
int main(int argc, char **argv) {
  ...
  sbr_vk_instance *sbr_vk_instance =
      sbr_vk_instance_create(sbr, sbr_vk_entry, vk.instance, &instance_params);
  if (!sbr_vk_instance)
  ...
  sbr_vk_rasterizer *rasterizer = sbr_vk_rasterizer_create(sbr, sbr_device);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    sbr_renderer_render_to(
        renderer, &ctx, subs, t, (sbr_rasterizer *)rasterizer,
