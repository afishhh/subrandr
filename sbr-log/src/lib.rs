...
impl LogFilter {
    ...
    fn try_from_env() -> Option<Self> {
        Some(LogFilter {
            top_level: std::env::var("SBR_LOG").ok().and_then(|x| x.parse().ok())?,
        })
    }
}
...
static ENV_LOG_FILTER: LazyLock<LogFilter> =
    LazyLock::new(|| LogFilter::try_from_env().unwrap_or_default());

fn log_default(level: Level, fmt: std::fmt::Arguments, module_path: &str) {
...
impl MessageCallback {
    fn log(&self, level: Level, fmt: std::fmt::Arguments, module_path: &str) {
        ...
        const INTERAL_CRATE_NAME_PREFIX: &str = "sbr_";

        let module_rel = {
            let tmp = module_path
                .strip_prefix(CRATE_MODULE_PREFIX)
                .unwrap_or(module_path);
            tmp.strip_prefix(INTERAL_CRATE_NAME_PREFIX).unwrap_or(tmp)
        };

        match self {
            Self::Default => {
                if !ENV_LOG_FILTER.filter(level) {
                    return;
                }
