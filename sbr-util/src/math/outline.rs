...
pub trait Outline<N: Number> {
    fn events(&self) -> impl Iterator<Item = OutlineEvent<N>>;

    fn control_box(&self) -> Rect2<N> {
        self.events().fold(Rect2::NOTHING, |mut r, e| {
            match e {
...
impl<N> OutlineEvent<N> {
    pub fn map<M>(self, mapper: impl Fn(Point2<N>) -> Point2<M>) -> OutlineEvent<M> {
        match self {
...
pub trait FloatOutlineIterExt: Iterator<Item = OutlineEvent<f32>> + Sized {
    // This is a pain to make a real iterator with the current non-owned flatenning APIs.
...
macro_rules! make_static_outline {
    ...
    (@contour_length($result: ident) move_to $first: expr; $($command: ident $($p: expr),+;)*) => {
        1
            $(+ $crate::make_static_outline!(@one $command $($p),*))*
