...
impl<N: Node> RedBlackTree<N> {
    ...
    fn find_by_raw(
        ...
        loop {
            match cmp(unsafe { Cursor::from_raw(current) }) {
                Ordering::Less => {
    ...
    pub fn find_by(
        ...
        match self.find_by_raw(cmp) {
            Ok(raw) => Ok(unsafe { Cursor::from_raw(raw) }),
            Err(InsertPosRaw::Before(raw)) => {
                Err(InsertPos::Before(unsafe { Cursor::from_raw(raw) }))
            }
    ...
    pub fn first(&self) -> Option<Cursor<'_, N>> {
        self.edge_raw(Direction::Left)
            .map(|raw| unsafe { Cursor::from_raw(raw) })
    }
...
impl<'t, N: Node> Cursor<'t, N> {
    unsafe fn from_raw(raw: CursorRaw<N>) -> Self {
        Self {
            tree: PhantomData,
            raw,
        }
...
impl<'t, N: Node> EndCursor<'t, N> {
    ...
    pub fn prev(self) -> Option<Cursor<'t, N>> {
        self.raw.map(|raw| unsafe { Cursor::from_raw(raw) })
    }
}
