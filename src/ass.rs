use core::str;
use std::{collections::BTreeMap, ops::Range, str::FromStr, sync::LazyLock};
...
fn parse_ass_color(value: &str) -> u32 {
...
fn parse_style_line(version: Version, line: &str) -> Style {
    ...
    build! {
        generic {
            ...
            weight: if values[7] == "-1" { 400 } else { 200 },
            italic: values[8] == "-1",
...
pub fn segment_event_text(text: &str) -> Vec<TextPart> {
    let mut result = vec![];
    ...
    loop {
            ...
            })
        {
            ...
            result.push(TextPart::Commands(start..end));
            current = end + 1;
        } else {
            if current < text.len() {
...
pub fn parse_ass(text: &str) -> Result<Script, Error> {
