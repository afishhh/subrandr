...
impl CAdapter {
    pub fn with_hal<R>(&self, callback: impl FnOnce(&wgpu::hal::vulkan::Adapter) -> R) -> R {
        unsafe {
            self.wgpu
                .as_hal::<wgpu::hal::vulkan::Api, _, R>(|hal| callback(hal.unwrap()))
        }
    }
}
...
unsafe extern "C" fn sbr_vk_adapter_required_physical_device_features(
    ...
    adapter.with_hal(|hal| {
        let exts = hal.required_device_extensions(wgpu::Features::default());
        let feat = hal.physical_device_features(&exts, wgpu::Features::empty());
...
unsafe extern "C" fn sbr_vk_device_from_raw(
    ...
    let hal_device = ctry!(adapter
        .wgpu
        .as_hal::<wgpu::hal::vulkan::Api, _, _>(|hal| hal.unwrap().device_from_raw(
            ash::Device::load(
                adapter
                    .instance
...
#[unsafe(no_mangle)]
unsafe extern "C" fn sbr_vk_rasterizer_create(
    _sbr: *mut Subrandr,
    cdevice: &CDevice,
) -> *mut CVkRasterizer {
    let storage =
