...
use crate::csssyn::tokenizer::TokenizerError;
...
impl Delimiter {
    pub(super) fn closing_str(self) -> &'static str {
        match self {
...
#[derive(Debug, Error)]
pub enum BufferTokenizationError {
    #[error("Source exceeds max length of {SOURCE_LEN_LIMIT} bytes")]
    SourceTooLong,
    #[error(transparent)]
    Tokenizer(#[from] TokenizerError),
}

impl From<BufferTokenizationError> for ParseError {
    fn from(value: BufferTokenizationError) -> Self {
        ParseError::new(
            Span {
                start: SOURCE_LEN_LIMIT,
                end: SOURCE_LEN_LIMIT,
            },
            value,
        )
    }
...
impl<'a> TokenBuffer<'a> {
    pub fn from_source(source: &'a str) -> Result<Self, BufferTokenizationError> {
        let Some(source_end) = u32::try_from(source.len())
            ...
            return Err(BufferTokenizationError::SourceTooLong);
        };
        ...
        for result in tokenize(source) {
            ...
            let mut close_stack = |expected_delimiter: Delimiter| {
                ...
                // TODO: `strict_sub` (MSRV 1.91)
                let end =
                    NonZero::new(u32::try_from(entries.len().checked_sub(idx).unwrap()).unwrap())
                        .unwrap();
            ...
            let entry_kind = match token.kind {
                ...
                TokenKind::Function => {
                    ...
                    EntryTokenKind::Function(GroupStart { end_offset: None })
                }
                TokenKind::LParen => {
                    group_stack.push((Delimiter::Parenthesis, entries.len()));
...
impl<'a> Cursor<'a> {
    ...
    pub(super) fn token(self) -> Option<(TokenView<'a>, Cursor<'a>)> {
        ...
        let token_source = unsafe { self.span_source(entry.span) };
    ...
    #[inline]
    #[must_use]
    pub fn outer_delimiter(self) -> Option<Delimiter> {
        match self.end().entry().kind {
            EntryTokenKind::CloseParenthesis => Some(Delimiter::Parenthesis),
            EntryTokenKind::CloseBracket => Some(Delimiter::Bracket),
            EntryTokenKind::CloseBrace => Some(Delimiter::Brace),
            _ => None,
        }
    }
}

// Cursor is entirely immutable so sharing it across threads is fine.
unsafe impl Send for Cursor<'_> {}
unsafe impl Sync for Cursor<'_> {}
