    ...
    fn enter_transformable_box(
        ...
        if let Some(Transform(rotation)) = style.transform() {
            if rotation != Rotation::None {
                let origin = fbox.border_box().size() / FixedL::new(2);
                output.apply_translation(origin);
                output.apply_rotation(rotation);
                output.apply_translation(-origin);
            }
        }
...
impl DisplayContext<'_> {
    ...
    fn display_background_at(
        ...
        if style.visibility().is_visible() && background.a > 0 {
            ...
            // TODO: Now that we support writing modes the distinction for x/y no longer makes sense
