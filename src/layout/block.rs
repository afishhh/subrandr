...
impl BlockInlineSizes {
    ...
    fn floating_shrink_to_fit_width(
        ...
        let preferred_width = content.max_content_width(lctx)?;
        // TODO: minimum width
...
impl PartialBlockContainerContent<'_> {
    fn max_content_width(&self, lctx: &mut LayoutContext) -> Result<FixedL, InlineLayoutError> {
        match self {
            PartialBlockContainerContent::Inline(inline) => inline.max_content_width(lctx),
            PartialBlockContainerContent::Block(children) => {
                let mut result = FixedL::ZERO;
                let infinite_containing_block = ContainingBlock::infinite_initial();

                for child in children {
                    result = result.max(child.inline_size_internal(
                        lctx,
                        BlockLayoutLevel::BlockLevel,
                        &infinite_containing_block,
                    )?);
                }
    ...
    pub(super) fn layout_internal(
        ...
        let content = match &self.content {
            ...
            PartialBlockContainerContent::Block(children) => {
                ...
                for child in children {
                    ...
                    let fragment = child.layout_internal(lctx, child_inline_sizes)?;
        ...
        Ok(BlockContainerFragment {
            ...
            fbox: FragmentBox::new_styled(
                Vec2::new(inline_sizes.width, height),
                lctx.dpi,
                &self.style,
                EdgeExtents {
                    // TODO: collapse margins or smth
                    top: self
                        .style
                        .margin_top()
