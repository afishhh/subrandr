...
use crate::{
    ...
    text::{self, FontArena, FontDb, FontMatcher, GlyphString, ShapingBuffer, TextMetrics},
};

use super::{FragmentBox, LayoutConstraints, LayoutContext, Vec2L};
...
pub struct RubyContainer {
    pub style: StyleMap,
    pub bases: Vec<InlineText>,
    pub annotations: Vec<RubyAnnotation2>,
}
...
pub(super) fn layout_inline(
    ...
    let font_arena = Rc::new(text::FontArena::new());
    ...
    for segment in container.contents.iter() {
        match segment {
            InlineChild::RubyContainer(ruby_container) => todo!(),
            InlineChild::Text(text_segment) => {
                let style = container_style.push(&text_segment.style);
                let matcher = text::FontMatcher::match_all(
                    ...
                    style.get_copy_or::<style::FontSize, _>(
                        I26Dot6::new(16) * context.dpi as i32 / 72,
                    ),
                    context.dpi,
                ...
                // match segment.ruby {
                //     Ruby::None => {
                shaper.add_text(&text_segment.text, matcher);
                //         }
                //         Ruby::Base => {
...
fn shape_simple_segment<'f>(
    ...
    fonts: &mut FontDb,
) -> Result<(Vec<text::Glyph<'f>>, TextMetrics), InlineLayoutError> {
...
impl<'f> MultilineTextShaper<'f> {
    ...
    pub fn shape(
        ...
        fonts: &mut FontDb,
    ) -> Result<(Vec<(Vec2L, Rc<LineBoxFragment>)>, Vec2L), InlineLayoutError> {
        while self
        ...
        let mut lines: Vec<(Vec2L, LineBoxFragment)> = Vec::new();
        ...
        while current_explicit_line <= self.explicit_line_bounaries.len() {
            ...
            type OutputLevelFragments = Vec<(Vec2L, TextFragment)>;

            let main_level: OutputLevelFragments = Vec::new();
            let annotation_levels: Vec<OutputLevelFragments> = Vec::new();
                            ...
                            .is_none_or(|split| *split >= end)
                        {
                            ...
                            main_level.push((
                                ...
                                TextFragment {
                                    ...
                                    baseline_offset: Vec2::new(ruby_padding, extents.max_ascender),
                                    glyphs,
                                    sty
            ...
            lines.push((
                line_rect.min.to_vec(),
                Rc::new(LineBoxFragment {
                    fbox: FragmentBox {
                        size: line_rect.size(),
                    },
                    children: segments,
