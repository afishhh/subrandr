...
use crate::{
    style::{
        computed::{
            self, FontSlant, InlineSizing, TextAlign, TextOrientation, ToPhysicalPixels,
...
impl RunShaper<'_> {
    fn set_buffer_content(
        ...
        let set_default_font_features = |buffer: &mut ShapingBuffer| match vertical_typesetting {
            Some(VerticalTypesetting::Upright) => buffer.set_feature(OpenTypeTag::FEAT_VERT, 1),
            Some(VerticalTypesetting::Sideways { .. }) => {
                buffer.set_feature(OpenTypeTag::FEAT_VRTR, 1)
            }
            None => (),
        };

        set_default_font_features(self.buffer);
                ...
                match self.font_feature_events.get(next_feature_ev_idx) {
                    Some(event) if event.utf8_index <= current => {
                        match event.kind {
                            ...
                            FontFeatureEventKind::Reset => {
                                self.buffer.reset_features();
                                set_default_font_features(self.buffer);
                            }
                        }
