...
use super::{Fixed2, IFixed24Dot8, Segment};
...
impl WindingTree {
    ...
    /// Adds a new edge that starts at `vertex` (whose id is `vertex_id`) with
    /// the specified effect on winding.
    /// This can also be used for removing an edge if the winding is specified
    /// to be the negation of the original winding of the edge.
    ...
    pub fn before(&self, p: Fixed2) -> Option<i32> {
        self.tree
            .exclusive_upper_bound_by(|k| {
                super::point_left_right_det(k.lower, k.upper, p).cmp(&IFixed24Dot8::ZERO)
            })
            .map(|(_, _, m)| m)
    }
