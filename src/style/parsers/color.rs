...
use super::*;
use crate::{csssyn::token::*, style::computed::Color};

// https://drafts.csswg.org/css-color-5/#typedef-color
pub(super) fn try_take_color(stream: &mut ParseStream) -> Result<Option<Color>, ParseError> {
    if stream.peek_skip("currentcolor") {
        // https://www.w3.org/TR/css-color-4/#resolving-other-colors
        // The currentcolor keyword computes to itself.
        Ok(Some(Color::CurrentColor))
    } else {
        try_take_color_base(stream)
    }
}

pub(super) fn take_color(stream: &mut ParseStream) -> Result<Color, ParseError> {
    try_take_color(stream)?.ok_or_else(|| stream.lookahead_error())
}

pub(super) fn take_color_for_color_property(
    stream: &mut ParseStream,
    parent: &ComputedStyle,
) -> Result<BGRA8, ParseError> {
    Ok(match take_color(stream)? {
        // https://www.w3.org/TR/css-color-4/#resolving-other-colors
        ...
        Color::CurrentColor => parent.color(),
        Color::Srgb(bgra8) => bgra8,
    })
}

// https://drafts.csswg.org/css-color-5/#typedef-color-base
fn try_take_color_base(stream: &mut ParseStream) -> Result<Option<Color>, ParseError> {
    if stream.peek(Hash) {
        ...
        Ok(Some(Color::Srgb(BGRA8::from_rgba32(value))))
        ...
        stream.skip();
        Ok(Some(Color::Srgb(NAMED_COLORS[found].1)))
    } else {
        try_take_color_function(stream)
    }
}

// https://drafts.csswg.org/css-color-5/#typedef-color-function
// only rgb{,a} for now
fn try_take_color_function(stream: &mut ParseStream<'_>) -> Result<Option<Color>, ParseError> {
    Ok(if stream.peek("rgb(") || stream.peek("rgba(") {
        let fun = stream.parse::<FunctionalNotation>()?;
        let content = parse_cursor::<RgbColorFunctionContent>(fun.content())?;
        Some(content.compute())
    } else {
        None
    })
}
...
mod test {
    ...
    fn compute_as_background_color(source: &str) -> Result<Color, ParseError> {
        test_parse_and_compute_str::<ComputedBackgroundColor>(source, take_color)
    }
    ...
    fn hex() {
        assert_eq!(
            ...
            Color::Srgb(BGRA8::from_rgba32(0xAABBCCFF))
        );

        assert_eq!(
            compute_as_background_color(r#"#ABCD"#).unwrap(),
            Color::Srgb(BGRA8::from_rgba32(0xAABBCCDD))
        );
        ...
        assert_eq!(
            ...
            Color::Srgb(BGRA8::from_rgba32(0xABCDDCBA))
        );
    }
    ...
    fn modern_rgb_functions() {
        assert_eq!(
            ...
            Color::Srgb(BGRA8::from_rgba32(0x008000C0))
        );
        assert_eq!(
            compute_as_background_color("rgba(200 20 100 / 0.25)").unwrap(),
            Color::Srgb(BGRA8::from_rgba32(0xC8146440))
        );
    ...
    fn named_colors() {
        for &(name, value) in NAMED_COLORS {
            assert_eq!(
                compute_as_background_color(name).unwrap(),
                Color::Srgb(value)
            );
        }
    }
