...
// FIXME: this generates a ton of braces (most probably not necessary)
...
pub fn check_inline(
    ...
    if expected_pixel_hash.as_deref() == Some(&pixel_hash_str) {
        ...
        if expected_pixel_hash.is_none() {
            eprintln!("Commit new snapshot with `cargo xxtask ptr write {display_path}`",);
        } else {
            todo!();
        }
